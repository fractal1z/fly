...
class Controller:
    ...
    def pose_callback(self, msg):
        self.center_pos = [msg.pose.position.x, msg.pose.position.y+0.1, msg.pose.position.z+0.6]

    def get_center_pos(self):
        self.circle_center_pos = [self.center_pos[0] - 0.1, self.center_pos[1], self.center_pos[2]]
        print(f"Calculated circle_center_pos: {self.circle_center_pos}")
        self.center_pos_end = [self.center_pos[0], self.center_pos[1], self.center_pos[2]]
        print(f"Calculated center_pos_end: {self.center_pos_end}")
    def load_params(self):
        self.state = 1
        self.takeoff_pub_point = rospy.get_param('~takeoff')
        self.takeoff_check_point = [0, 0, 1.2]
    ...
    def is_close(self, odom: Odometry, point: list):
        if (self.state == 0):
            if (odom.pose.pose.position.z - point[2])**2 < 0.1:
                return True
            return False
        elif (self.state == 9):
    ...
    def odom_cb(self, msg: Odometry):
        ...
        # state 0 : takeoff
        if self.state == 0:
            if self.is_close(msg, self.takeoff_check_point):
                rospy.sleep(self.sleep_time)
                self.state = 1
                pass
            else:
                self.pub(self.takeoff_pub_point)
                # print(f"STATE:{self.state},pub_point:{self.takeoff_point}")
        # state 1 : first_square_point_pre
        if self.state == 1:
